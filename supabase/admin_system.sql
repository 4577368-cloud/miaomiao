...
BEGIN
  ...
  -- 更新用户状态
  UPDATE profiles 
  SET status = new_status::user_status, updated_at = NOW()
  WHERE id = p_user_id;
...
BEGIN
  ...
  SELECT 
    ...
    sp.phone as sitter_phone,
    o.created_at
  FROM orders o
  LEFT JOIN profiles op ON o.owner_id = op.id
...
-- 创建用户状态枚举类型
CREATE TYPE IF NOT EXISTS user_status AS ENUM ('active', 'banned');
