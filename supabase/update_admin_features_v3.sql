...
DROP FUNCTION IF EXISTS admin_get_banners();
CREATE OR REPLACE FUNCTION admin_get_banners()
RETURNS TABLE (
  id UUID,
  title VARCHAR,
  image_url TEXT,
...
DROP FUNCTION IF EXISTS admin_upsert_banner(UUID, VARCHAR, TEXT, VARCHAR, JSONB, BOOLEAN, INT, TIMESTAMPTZ, TIMESTAMPTZ);
...
BEGIN
  IF p_id IS NULL THEN
    INSERT INTO banners (title, image_url, action_type, action_payload, is_active, sort_order, start_time, end_time)
...
DROP FUNCTION IF EXISTS admin_delete_banner(UUID);
CREATE OR REPLACE FUNCTION admin_delete_banner(p_id UUID)
RETURNS BOOLEAN AS $$
BEGIN
